{ self, ... }:
{
  den.aspects.apps.provides.development.provides.versioning.provides.git.provides.forgejo-cli = {
    description = "Forgejo CLI";

    homeManager = {
      imports = [ self.homeManagerModules.forgejo-cli ];

      programs.forgejo-cli.enable = true;
    };
    ...
    provides.add-host =
      ...
      {
        description = "Add a new host to the Forgejo CLI config";
        homeManager =
          { config, lib, ... }:
          let
            secret = "${hostname}_${username}-token";
          in
          {
            sops.secrets.${secret} = {
              inherit sopsFile;
            ...
            home.shellAliases = lib.mkIf (alias == null) {
              ${alias} = "fj --host ${hostname}";
            };
          };
      };
  };
