# from https://github.com/NixOS/nixpkgs/blob/2c9b9ae88a3ac46066caa91a83552768d270a6e1/nixos/modules/programs/nix-ld.nix to support 32bit
...
let
  ...
  cfg = config.programs.nix-ld-local.systems;
...
in
{
  imports =
    ...
    [
      (mkAliasOptionModule
        [ "programs" "nix-ld-local" "enable" ]
        [ "programs" "nix-ld-local" "systems" system "enable" ]
      )
      (mkAliasOptionModule
        [ "programs" "nix-ld-local" "package" ]
        [ "programs" "nix-ld-local" "systems" system "package" ]
      )
      (mkAliasOptionModule
        [ "programs" "nix-ld-local" "libraries" ]
        [ "programs" "nix-ld-local" "systems" system "libraries" ]
      )
    ];

  options.programs.nix-ld-local.systems = mkOption {
    default = { };
    description = ''
